...
1. Read the one-sentence summary and the phase flow above.
2. Open `Vortex_DSE_CSlot_AE.tla` and locate `MerkleAgreement` (for example: `grep -n "MerkleAgreement" Vortex_DSE_CSlot_AE.tla`).
3. Check assumptions and bounded results in `STATUS.md`.
4. Run TLC locally (see below).
5. Run Apalache locally (see below).
